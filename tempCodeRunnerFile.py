...
async def on_ready():
    ...
    # Start background task to sync data every 5 minutes
    asyncio.create_task(sync_data())  # Updated for discord.py v2.x
...
# Function to sync data every 5 minutes
async def sync_data():
    while True:
        await asyncio.sleep(300)  # Wait for 5 minutes (300 seconds)
        global orders, updated_ips, ip_status_data
        orders = load_orders()         # Reload the order data
        updated_ips = load_updated_ips()  # Reload the updated IP data
        ip_status_data = load_ip_status()  # Reload the IP status data
